...
用途：为账号体系与会员体系提供统一的数据模型接口。
当前采用 SQLite，适合本地开发与原型验证；正式生产环境应迁移至托管数据库。
不保存明文密码，不保存支付敏感信息。
"""
...
def upsert_user(email: str, display_name: str = "") -> Dict[str, Any]:
    ...
    with _connect() as conn:
        ...
        row = conn.execute(
            "SELECT user_id,email,display_name,created_at,updated_at FROM users WHERE email=?",
            (email,),
        ).fetchone()
...
def get_membership(user_id: str) -> Dict[str, Any]:
    ...
    with _connect() as conn:
        row = conn.execute(
            "SELECT user_id,plan,status,expires_at,updated_at FROM memberships WHERE user_id=?",
            (str(user_id),),
        ).fetchone()
    return dict(row) if row else {
        "user_id": str(user_id),
    ...
    with _connect() as conn:
        conn.execute(
            ...
            INSERT INTO research_history(
                user_id,code,name,score,decision,price,normal_value,safety_margin,created_at
            ) VALUES(?,?,?,?,?,?,?,?,?)
            """,
            (
                str(user_id),
                str(snapshot.get("code", "")),
                str(snapshot.get("name", "")),
                snapshot.get("score"),
                str(snapshot.get("decision", "")),
                snapshot.get("price"),
                snapshot.get("normal_value"),
                snapshot.get("safety_margin"),
                now,
            ),
        )
