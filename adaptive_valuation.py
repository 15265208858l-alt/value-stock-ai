...
2. AI/算力、服务器、通信设备、半导体等高成长科技公司优先使用成长PE+PB。
3. 成长科技估值采用TTM/正常化EPS，不把未经验证的未来利润直接当成事实。
4. Forward EPS只做观察指标。
...
def detect_valuation_model(industry=None, market_industry=None, stock_code=None, override="自动识别"):
    ...
    growth_tech_codes = {
        "001339", "300308", "300502", "300394", "000938", "000977", "601138", "688041", "603019",
...
def get_valuation_config(model, annual_roe=None):
    ...
    if model == "growth_tech":
        ...
        note = (
            f"V16.8成长科技模型：估值优先采用{earnings_basis.get('basis', 'FY年度EPS')}，"
            "不直接把未经验证的未来利润当作事实。"
        )
