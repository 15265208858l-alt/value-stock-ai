"""A股价值研投｜我的股票池 V8
商业层模块：账号持久化 + 最近研究恢复 + 手动轻量行情刷新 + 免费版智能研究排行预览。
不修改核心研究引擎。免费用户可查看最近研究 Top 3；Pro 才可使用完整股票池、持续跟踪与管理功能。
"""
...
from user_store import (
    add_watchlist_stock, clear_watchlist as clear_db_watchlist,
    get_watchlist as get_db_watchlist, remove_watchlist_stock,
    latest_research_for_codes, recent_research,
)
...
def get_watchlist()->List[str]:
    uid=_user_id()
    if uid:
        x=get_db_watchlist(uid,MAX_STOCKS)
        st.session_state[WATCHLIST_KEY]=x
        return x
    x=st.session_state.get(WATCHLIST_KEY,[])
    return [str(v) for v in x if re.fullmatch(r"\d{6}",str(v))]
...
def record_research_snapshot(**kwargs)->None:
    ...
    s=_snapshots()
    s[code]={
        "code":code,"name":str(kwargs.get("name") or code),"price":kwargs.get("price"),
        "score":kwargs.get("score"),"rating":str(kwargs.get("rating") or "暂无"),
        "decision":str(kwargs.get("decision") or "暂无"),"action":str(kwargs.get("action") or "暂无"),
        "position":str(kwargs.get("position") or "暂无"),"normal_value":kwargs.get("normal_value"),
        "safety_margin":kwargs.get("safety_margin"),"valuation_level":str(kwargs.get("valuation_level") or "数据不足"),
        "historical_level":str(kwargs.get("historical_level") or "数据不足"),"risk_level":str(kwargs.get("risk_level") or "数据不足")
    }
    st.session_state[SNAPSHOT_KEY]=s
...
def _load_persisted_snapshots(items:List[str])->Dict[str,Dict[str,Any]]:
    ...
    if uid:
        for code,row in latest_research_for_codes(uid,items).items():
            if code not in s:
                s[code]={
                    "code":code,"name":row.get("name") or code,"price":row.get("price"),
                    "score":row.get("score"),"rating":"暂无","decision":row.get("decision") or "暂无",
                    "action":"暂无","position":"暂无","normal_value":row.get("normal_value"),
                    "safety_margin":row.get("safety_margin"),"valuation_level":"暂无",
                    "historical_level":"暂无","risk_level":"暂无"
                }
    st.session_state[SNAPSHOT_KEY]=s
    return s
...
def _quote(code):
    cache=st.session_state.setdefault(QUOTE_CACHE_KEY,{})
    old=cache.get(code);now=time.time()
    if old and now-old[0]<QUOTE_TTL:return old[1]
    try:
        data=load_stock_data_fast(code)
        q={"price":get_latest_price(data.get("history")) if data else None,
           "name":(data.get("market") or {}).get("名称",code) if data else code}
...
def _alert_editor(code,s):
    ...
    with c:
        if st.button("保存",key=f"vs_alert_save_{code}",use_container_width=True):
            set_alert(code,_to_float(entry),_to_float(heavy));st.success("✅ 已保存");st.rerun()
    with d:
        if st.button("删除",key=f"vs_alert_del_{code}",use_container_width=True):
            remove_alert(code);st.success("✅ 已删除");st.rerun()
...
def _research_rank(rows:List[Dict[str,Any]])->List[Dict[str,Any]]:
    """免费版只使用已有研究历史，不重新跑核心研究；按安全边际优先、评分辅助排序。"""
    latest={}
    for row in rows or []:
    ...
    for row in latest.values():
        ...
        rank=(margin if margin is not None else -999.0)*0.6+(score if score is not None else 0.0)*0.4
        row["rank_score"]=rank
        row["rank_margin"]=margin
        ranked.append(row)
    ranked.sort(key=lambda x:x.get("rank_score",-999),reverse=True)
    return ranked


def render_free_rank_preview():
    """V8：免费用户可直接看到最近研究股票 Top 3，解决无法测试 Pro 股票池的问题。"""
    uid=_user_id()
    if not uid or is_pro():return
    rows=_research_rank(recent_research(uid,limit=20))
    st.markdown("### 🏆 最近研究智能排行")
    st.caption("免费版展示最近研究股票 Top 3；不重新运行完整研究，不消耗额外数据链路。")
    if not rows:
        st.info("📌 先完成至少1只股票的价值研究，这里会自动形成你的智能排行。")
        return
    preview=[]
    for i,row in enumerate(rows[:3],1):
        preview.append({
            ...
            "安全边际":_fmt(row.get("safety_margin"),"%"),
            "合理价":_fmt(row.get("normal_value")),
            "当前价":_fmt(row.get("price")),
...
def render_watchlist_dashboard():
    ...
    if items:
        if is_pro():
            ...
            with top2:
                if st.button("🔄 刷新行情",key="vs_wl_v8_refresh",use_container_width=True):
                    with st.spinner("正在刷新股票池行情…"):n=refresh_quotes(items)
                    st.success(f"已更新 {n}/{len(items)} 只股票");st.rerun()
        rows=[]
        for code in items:
            s=snaps.get(code,{})
            q=st.session_state.get(QUOTE_CACHE_KEY,{}).get(code,({},{}))[1]
            price=q.get("price") or s.get("price")
            normal=s.get("normal_value")
            margin=None if price is None or normal in (None,0) else (float(normal)/float(price)-1)*100
        ...
        if is_pro():st.caption("⚡ 行情刷新使用现有 fast_data 缓存；不会重新执行20只股票的完整价值研究。")
        with st.expander("🔔 设置价格提醒"):
            target=st.selectbox("选择股票",items,format_func=lambda x:f"{snaps.get(x,{}).get('name',x)} ({x})",key="vs_alert_target_v8")
            _alert_editor(target,snaps.get(target,{}))
        with st.expander("📄 生成研究报告"):
            target=st.selectbox("选择股票",items,format_func=lambda x:f"{snaps.get(x,{}).get('name',x)} ({x})",key="vs_report_target_v8")
            render_research_report_panel(target,snaps.get(target,{}))
        if is_pro() and st.button("清空股票池",key="vs_wl_v8_clear",use_container_width=True):
            clear_watchlist();st.rerun()
    else:
    ...
    with st.expander("＋ 添加/移除股票"):
        if not is_pro():
            ...
            code=st.text_input("股票代码",placeholder="例如：000333",key="vs_wl_v8_code")
            c1,c2=st.columns(2)
            with c1:
                if st.button("加入股票池",key="vs_wl_v8_add",use_container_width=True):
                    if add_stock(code):st.success("✅ 已加入并保存到账号");st.rerun()
                    else:st.warning(f"请输入有效6位A股代码，股票池最多{MAX_STOCKS}只。")
            with c2:
                rm=st.text_input("移除代码",placeholder="例如：000333",key="vs_wl_v8_rm")
