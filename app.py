...
if analyze:
    ...
    dc2.metric(
        "已获取模块",
        f"{completeness['available']}/{completeness['total']}"
    )
    ...
    financial_quality = (
        calculate_financial_quality(
            trend,
            cash_result.get("cashflow", {}).get("ratio")
        )
    )
    ...
    if valuation_model_override != "自动识别":
        valuation_model = (
            detect_valuation_model(
                stock_code=stock_code,
    ...
    st.write(
        ...
        + (
            ...
            else f"{conservative_value:.2f} 元"
        )
        + " ｜ 乐观价值："
        + (
            "暂无"
            if optimistic_value is None
            else f"{optimistic_value:.2f} 元"
        )
        + " ｜ 重仓参考价："
        + (
            "暂无"
    ...
    if (
        ...
        h6.metric(
            ...
            if historical_stats[
                ...
                f"{historical_stats['percentile']:.1f}%"
            )
        )
