...
def get_report_metrics(profit, balance, cashflow):
    return {
        "revenue": get_latest_report_value(
            profit, ["营业总收入", "营业收入", "一、营业总收入"]
        ),
        "net_profit": get_latest_report_value(
            profit,
            ["归属于母公司所有者的净利润", "归属于母公司股东的净利润", "净利润", "五、净利润"]
        ),
        "receivable": get_latest_report_value(
            balance, ["应收账款", "应收款项"]
        ),
        "inventory": get_latest_report_value(balance, ["存货"]),
...
if analyze:
    ...
    st.metric("财务风险评分", f"{risk_score}/10")
    ...
    cashflow_ratio = (
        risk_result.get("cashflow", {}).get("ratio")
    )
    ...
    valuation_model_override = st.selectbox(
        "🧠 估值模型（默认自动识别，可手动调整）",
        ["自动识别", "普通成长/制造", "银行", "保险", "券商", "周期"],
        index=0,
        key="valuation_model_override_v165"
    )
    ...
    historical_pe = build_historical_pe(history, trend, max_years=10)
    historical_stats = calculate_historical_statistics(historical_pe, current_pe)
    historical_level = get_historical_valuation_level(historical_stats["percentile"])
    ...
    if historical_pe is not None and not historical_pe.empty:
        ...
        h1.metric("历史最低PE", "暂无" if historical_stats["min"] is None else f"{historical_stats['min']:.2f}")
        h2.metric("历史中位PE", "暂无" if historical_stats["median"] is None else f"{historical_stats['median']:.2f}")
        h3.metric("历史最高PE", "暂无" if historical_stats["max"] is None else f"{historical_stats['max']:.2f}")

        h4, h5, h6 = st.columns(3)
        h4.metric("历史25%分位", "暂无" if historical_stats["q25"] is None else f"{historical_stats['q25']:.2f}")
        h5.metric("历史75%分位", "暂无" if historical_stats["q75"] is None else f"{historical_stats['q75']:.2f}")
        h6.metric("当前PE历史分位", "暂无" if historical_stats["percentile"] is None else f"{historical_stats['percentile']:.1f}%")

        if historical_stats["deviation"] is not None:
            st.metric("当前PE相对历史中位数偏离", f"{historical_stats['deviation']:.2f}%")
    ...
    auto_peer_result = get_peer_candidates(stock_code, max_peers=5)
    ...
    if len(peer_codes) < 2:
        ...
        if len(peer_rows) < 2:
            ...
            for column in ["价格", "ROE", "营收增长率", "净利润增长率", "资产负债率", "PE", "PB"]:
            ...
            if peer_score_result["details"]:
                st.subheader("📋 同行评分明细")
                st.dataframe(
                    pd.DataFrame(peer_score_result["details"]),
    ...
    score_table = pd.DataFrame({
        "分析维度": ["财务质量", "同行竞争力", "当前估值", "历史估值", "风险控制"],
        "满分": [30, 25, 20, 15, 10],
        "实际得分": [
